...
export const FollowingUserCard = ({ FollowUser }: FollowUserProps) => {
  const navigate = useNavigate();
  const currentUser = useAuthStore((state) => state.user);

  const [isFollow, setIsFollow] = useState(FollowUser.isFollow);
  const toggleFollow = useToggleFollow(FollowUser.following.id);
  ...
  const isCurrentUser = currentUser?.id === FollowUser.following.id;
